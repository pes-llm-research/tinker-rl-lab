...
Prior RL post-training comparisons between PPO and GRPO are usually
reported on different model versions, training horizons, or hardware,
making direct algorithm-level comparison difficult. We provide a
\emph{same-task, same-checkpoint, fixed-step descriptive} comparison
using the Modal H100 cluster for PPO and the Tinker API for GRPO.
Because the two arms use different backends and launchers, this is
\emph{not} an isolated algorithm-only estimate; we read the rows below
as stack-level PPO/GRPO case studies, not algorithm-causal evidence.
...
(3) On Llama-3.1-8B, PPO has the higher last-10 mean ($97.5\%$ vs.\
$84.4\%$). At $s_g{=}1$ per cell across the two backends the
per-step Mann-Whitney comparison is descriptive only
(MW $p \approx 0.006$, Bonferroni-corrected $p \approx 0.012$ on the
per-step ranks within this single seed-pair); we do \emph{not} read it
as a seed-level algorithm-causal result, and we retract the
earlier $p < 10^{-10}$ figure that arose from treating per-step
observations as i.i.d.\ across runs;
(4) Algorithm-ranking is descriptive at this seed budget. On Qwen3-8B,
PPO and GRPO are statistically indistinguishable on last-10 reward
(Cohen's $d = 0.166$, negligible); GRPO's lower per-step variance
(CV 0.46 vs.\ 1.00) is the only practical contrast. On Llama-3.1-8B,
PPO's last-10 point estimate is higher, but the comparison is
single-seed and across backends. We therefore do \emph{not} claim
``GRPO is preferred for Qwen3-8B / PPO is preferred for Llama-3.1-8B''
as an algorithmic recommendation; we describe the heterogeneity and
defer the algorithmic question to multi-seed same-stack replication.
...
LLM-native stacks vs Classic RL stacks\footnote{The Cohen's $d$
reported here ($21.84$) and the $+22.436$ value reported in the
Statistical Rigor Addendum's survival table are computed on slightly
different cell selections of the same underlying $5$-seed TRL-GRPO
reference and $15$-run Classic-RL pool: the legacy table above pools
the Tinker-managed last-10 reward across all completed Tinker rows
into the LLM-native column, while the survival table restricts the
LLM-native column to the matched-protocol TRL-only Tier-A subset.
Both numbers are large effect-size signatures of structural failure of
classic-RL defaults on LLM arithmetic, and the addendum's $+22.436$ is
the value the paper now stands behind.} & $5$ & $15$ & $21.84$ &
$[14.63, 29.04]$ & $<$0.001$^{*\dag}$ & $1.00$ & $1.45$ \\
...
GSM8K, single-seed, 30 training steps over $G \in \{2, 4, 8, 16\}$
(matching the Group-Size block of Table~\ref{tab:main_results}).
Blue bars show peak reward; vermillion bars show last-10 average
reward. Peak reward and last-10 mean both rise from $G{=}2$
(peak 50.0\%, last-10 37.5\%) through $G{=}4$ (peak 75.0\%, last-10
52.1\%) to $G{=}8$ (peak 100\%, last-10 84.4\%) and then fall at
$G{=}16$ (peak 71.9\%, last-10 38.0\%). $G{=}8$ is the
single-seed in-budget optimum on this Qwen3-8B sweep, with $G{=}4$
second on the last-10 metric; the inverted-U pattern at $G{=}16$
suggests that too many group members dilute the signal from the few
correct completions. We do \emph{not} claim $G{=}8$ is universally
optimal: this is one model, one task, one seed.}
...
The sharpest pattern in our benchmark is a \emph{model-dependent}
PPO/GRPO interaction. On Llama-3.1-8B (TRL, single-seed), PPO's
last-10 reward is higher than GRPO's point estimate; on Qwen3-8B
(same TRL stack), PPO and GRPO are \emph{statistically
indistinguishable} (Cohen's $d \approx 0.17$, negligible).
We therefore do not claim ``GRPO outperforms PPO on Qwen3-8B'', and
the Llama-3.1-8B comparison is descriptive: at $s_g{=}1$ per cell
and different backend implementations and different reference
checkpoints (Tinker Qwen3-8B-Base vs.\ HF Qwen3-8B on the TRL
arm), a Mann-Whitney $p$-value is not a valid algorithm-only
inference (we previously quoted $r = 0.94$, $p < 10^{-10}$;
under this non-matched checkpoint setup that reported $p$-value
confounds instruction-tuning and backend effects with the
algorithmic contrast and should not be read as algorithm-causal).
The two arms share the same evaluation metric but \emph{not} the
same starting policy: a Base model and an Instruct model enter RL
with substantially different entropy, reward geometry, and
format-discovery burdens. We therefore treat the cross-checkpoint
pattern as descriptive, not an algorithmic estimate. We offer three complementary mechanistic hypotheses that
future gradient-level analysis can test.
