...
\tightlist
\item
  \textbf{Tool calling (agentic):} structured JSON function calling with
  5--60,000 tool schemas
\item
  \textbf{Code generation (transfer control):} HumanEval benchmark
  subset
\item
  \textbf{Mathematical reasoning (transfer control):} GSM8K
  (grade-school). Although MATH (competition-level) was part of our
  original scope, the MATH track did not reach the same experimental
  maturity as GSM8K; we therefore exclude it from our main claims and
  treat it as exploratory pilot work
\end{itemize}
...
\tightlist
\item
  \textbf{Steps 1-20:} Stuck at reward 0.10-0.18, 75-100\% ZVF
  (saturated at bottom)
\item
  \textbf{Steps 21-40:} Sudden breakout --- reward climbed 0.28
  -\textgreater{} \textbf{0.873}
\item
  \textbf{Step 41:} Catastrophic collapse --- reward crashed 0.87
  -\textgreater{} 0.002 -\textgreater{} \textbf{0.000}
\item
  \textbf{Steps 42-50:} Dead --- zero reward, 100\% ZVF
\end{enumerate}
...
updates between mid-2025 and April 2026 include:

\begin{itemize}
\tightlist
\item
  TRL v1.0--1.2: asynchronous GRPO, Liger memory reduction,
  VESPO/DPPO/SDPO, tool-calling support for Qwen3 and LLaMA 3.1/3.2
\item
  veRL v0.7.1: GSPO support, Megatron MoE backend, SGLang rollout,
  per-sample temperature, FlowGRPO trainer
\item
